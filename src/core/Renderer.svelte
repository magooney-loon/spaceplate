<script lang="ts">
	...
	const { scene, renderer, camera, size, autoRender, autoRenderTask } = useThrelte();

	const composer = new EffectComposer(renderer);
	// EffectComposer constructor permanently sets renderer.autoClear = false.
	// Restore it so Studio's tasks (RenderSelectedObjects, DefaultCamera) still
	// clear their render targets correctly. We re-disable it only during composer.render().
