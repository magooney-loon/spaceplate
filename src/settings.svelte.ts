...
const createLogger = (prefix: string, enabled: boolean) => ({
  ...
  warn:  (...args: unknown[]) => { if (enabled) console.warn (`[${prefix}]`, ...args); },
  error: (...args: unknown[]) => { if (enabled) console.error(`[${prefix}]`, ...args); },
});
...
export const audioActions = {
  toggleMusic()    { settingsState.audio.musicEnabled    = !settingsState.audio.musicEnabled;    log.info('Music:', settingsState.audio.musicEnabled ? 'on' : 'off'); },
  toggleEffects()  { settingsState.audio.effectsEnabled  = !settingsState.audio.effectsEnabled;  log.info('Effects:', settingsState.audio.effectsEnabled ? 'on' : 'off'); },
  toggleAmbience() { settingsState.audio.ambienceEnabled = !settingsState.audio.ambienceEnabled; log.info('Ambience:', settingsState.audio.ambienceEnabled ? 'on' : 'off'); },
  setMusicVolume(v: number)    { settingsState.audio.musicVolume    = v; toStorage(MUSIC_VOLUME_KEY,    String(v)); log.info('Music volume:', v); },
  setAmbienceVolume(v: number) { settingsState.audio.ambienceVolume = v; toStorage(AMBIENCE_VOLUME_KEY, String(v)); log.info('Ambience volume:', v); },
  setEffectsVolume(v: number)  { settingsState.audio.effectsVolume  = v; toStorage(EFFECTS_VOLUME_KEY,  String(v)); log.info('Effects volume:', v); },
};
