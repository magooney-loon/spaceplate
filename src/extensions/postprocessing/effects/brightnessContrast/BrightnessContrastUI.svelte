...
<Folder title="Brightness & Contrast" expanded={false}>
	<Checkbox
		value={state.enabled}
		label="Enabled"
		on:change={() => actions.toggleBrightnessContrast()}
	/>
	{#if state.enabled}
		<Slider
			...
			max={1}
			step={0.01}
			on:change={(e) => actions.setBrightness(e.detail.value)}
		/>
		<Slider
			...
			max={1}
			step={0.01}
			on:change={(e) => actions.setContrast(e.detail.value)}
		/>
		<List
			value={state.blendFunction}
			label="Blend Function"
			options={blendFunctionOptions}
			on:change={(e) => actions.setBrightnessContrastBlendFunction(e.detail.value as BlendFunction)}
		/>
		<Button title="Reset" on:click={actions.resetBrightnessContrast} />
	{/if}
</Folder>
