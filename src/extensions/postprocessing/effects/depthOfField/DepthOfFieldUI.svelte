...
<Folder title="Depth of Field" expanded={false}>
	<Checkbox value={state.enabled} label="Enabled" on:change={() => actions.toggleDepthOfField()} />
	{#if state.enabled}
		<Slider
			...
			max={100}
			step={0.1}
			on:change={(e) => actions.setDepthOfFieldFocusDistance(e.detail.value)}
		/>
		<Slider
			value={state.focusRange}
			label="Focus Range"
			min={0.1}
			max={50}
			step={0.1}
			on:change={(e) => actions.setDepthOfFieldFocusRange(e.detail.value)}
		/>
		<Slider
			value={state.bokehScale}
			label="Bokeh Scale"
			min={0.1}
			max={10}
			step={0.1}
		...
		<List
			...
			on:change={(e) => actions.setDepthOfFieldBlendFunction(e.detail.value as BlendFunction)}
		/>
		<Button title="Reset" on:click={actions.resetDepthOfField} />
	{/if}
</Folder>
