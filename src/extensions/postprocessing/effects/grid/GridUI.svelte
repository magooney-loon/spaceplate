...
<Folder title="Grid" expanded={false}>
	<Checkbox value={state.enabled} label="Enabled" on:change={() => actions.toggleGrid()} />
	{#if state.enabled}
		<Slider
			value={state.scale}
			label="Scale"
			...
			max={10}
			step={0.1}
			on:change={(e) => actions.setGridScale(e.detail.value)}
		/>
		<Slider
			...
			max={1}
			step={0.01}
			on:change={(e) => actions.setGridLineWidth(e.detail.value)}
		/>
		<List
			value={state.blendFunction}
			label="Blend Function"
			options={blendFunctionOptions}
			on:change={(e) => actions.setGridBlendFunction(e.detail.value as BlendFunction)}
		/>
		<Button title="Reset" on:click={actions.resetGrid} />
	{/if}
</Folder>
