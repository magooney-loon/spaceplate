...
<Folder title="Hue & Saturation" expanded={false}>
	<Checkbox value={state.enabled} label="Enabled" on:change={() => actions.toggleHueSaturation()} />
	{#if state.enabled}
		<Slider
			value={state.hue}
			label="Hue"
			...
			max={3.14}
			step={0.01}
			on:change={(e) => actions.setHue(e.detail.value)}
		/>
		<Slider
			...
			max={1}
			step={0.01}
			on:change={(e) => actions.setSaturation(e.detail.value)}
		/>
		<List
			value={state.blendFunction}
			label="Blend Function"
			options={blendFunctionOptions}
			on:change={(e) => actions.setHueSaturationBlendFunction(e.detail.value as BlendFunction)}
		/>
		<Button title="Reset" on:click={actions.resetHueSaturation} />
	{/if}
</Folder>
