...
<Folder title="Color Depth" expanded={false}>
	<Checkbox value={state.enabled} label="Enabled" on:change={() => actions.toggleColorDepth()} />
	{#if state.enabled}
		<Slider
			value={state.bits}
			label="Bits"
			...
			max={32}
			step={1}
			on:change={(e) => actions.setColorDepthBits(e.detail.value)}
		/>
		<List
			value={state.blendFunction}
			label="Blend Function"
			options={blendFunctionOptions}
			on:change={(e) => actions.setColorDepthBlendFunction(e.detail.value as BlendFunction)}
		/>
		<Button title="Reset" on:click={actions.resetColorDepth} />
	{/if}
</Folder>
