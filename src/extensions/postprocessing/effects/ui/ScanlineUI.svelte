...
<Folder title="Scanline" expanded={false}>
	<Checkbox value={state.enabled} label="Enabled" on:change={() => actions.toggleScanline()} />
	{#if state.enabled}
		<Slider
			value={state.density}
			label="Density"
			...
			max={5}
			step={0.1}
			on:change={(e) => actions.setScanlineDensity(e.detail.value)}
		/>
		<Slider
			value={state.opacity}
			label="Opacity"
			...
			max={1}
			step={0.01}
			on:change={(e) => actions.setScanlineOpacity(e.detail.value)}
		/>
		<Slider
			...
			max={10}
			step={0.1}
			on:change={(e) => actions.setScanlineScrollSpeed(e.detail.value)}
		/>
		<List
			value={state.blendFunction}
			label="Blend Function"
			options={blendFunctionOptions}
			on:change={(e) => actions.setScanlineBlendFunction(e.detail.value as BlendFunction)}
		/>
		<Button title="Reset" on:click={actions.resetScanline} />
	{/if}
</Folder>
