...
<Folder title="Depth Effect" expanded={false}>
	<Checkbox value={state.enabled} label="Enabled" on:change={() => actions.toggleDepthEffect()} />
	{#if state.enabled}
		<Checkbox value={state.inverted} label="Inverted" />
		<List
			value={state.blendFunction}
			label="Blend Function"
			options={blendFunctionOptions}
			on:change={(e) => actions.setDepthEffectBlendFunction(e.detail.value as BlendFunction)}
		/>
		<Button title="Reset" on:click={actions.resetDepthEffect} />
	{/if}
</Folder>
