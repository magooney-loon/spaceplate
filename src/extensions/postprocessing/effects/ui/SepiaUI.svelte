...
<Folder title="Sepia" expanded={false}>
	<Checkbox value={state.enabled} label="Enabled" on:change={() => actions.toggleSepia()} />
	{#if state.enabled}
		<Slider
			...
			max={1}
			step={0.01}
			on:change={(e) => actions.setSepiaIntensity(e.detail.value)}
		/>
		<List
			value={state.blendFunction}
			label="Blend Function"
			options={blendFunctionOptions}
			on:change={(e) => actions.setSepiaBlendFunction(e.detail.value as BlendFunction)}
		/>
		<Button title="Reset" on:click={actions.resetSepia} />
	{/if}
</Folder>
