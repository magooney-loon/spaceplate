...
- **`transitionTo(scene)` — the per-scene warm swap ("bootloader")**: the `goTo*`/`goBack` actions (every HUD button) route through it. **The cover is a FROZEN FRAME**, not a black screen: the post-processing composite captures the outgoing scene's last frame and holds it (`$core/postprocessing/transitionState.svelte`), the swap and the load happen behind it, and it dissolves into the new scene at the end. Loader.svelte's black veil is the fallback for when there is no pipeline to do that with. Cover → `setScene` → one rAF for the mount to flush, which is where the new scene's `useGltf`/`TextureLoader` calls fire → **the asset gate** (`waitForAssetsIdle`, `$core/utils/assetGate`) holds the veil until that loading queue drains → one rAF for the subtrees those assets gated to mount → **the warm gate** (`warmScene`, `$core/utils/warmup.svelte`) forces real frames of the real pipeline until three stops building shader programs → **the reveal** dissolves the frozen frame away. No warm frame is forced by hand — mount invalidations already draw it; this module lives outside the Canvas and has no Threlte context. Guards: same-scene and already-transitioning are no-ops. The boot scene needs none of this: it mounts at t=0 behind the Loader and compiles while assets stream.
  - **`isTransitioning` is raised AFTER the freeze, and the re-entrancy guard is a separate plain `busy` flag.** The capture needs two frames of the LIVE scene to grab, and that flag is what puts the fallback veil on screen — raising it first flashed black at the start of every transition. Which is the general shape: the flag means "a cover is warranted", not "a call is in flight".
