...
- **graphics**:
  ...
  - `renderScale: number` (0.5–1, default 1) — render resolution as a fraction of what the preset would otherwise use. **`App.svelte` multiplies it onto the preset's base DPR rather than replacing it**, so 1 changes nothing on either preset and the two knobs compose instead of fighting. Clamped in the action AND on load, so a hand-edited localStorage value cannot put the renderer at 0.
  - `maxFps: number` (one of `FPS_CAPS`: 0/30/60/120, default 0 = **VSync** — the monitor's refresh rate, which is the browser's ceiling anyway; no gate is installed). Frame-rate cap for the whole engine loop — tasks and renders alike — below that. Enforced by the frame gate in `core/utils/EngineClock.svelte`, which re-wraps the animation-loop callback (NOT `scheduler.run`: skipping inside the scheduler would let Threlte's closure swallow invalidations on skipped ticks). The effective rate snaps to whole refresh intervals (rAF only ticks on vsyncs — a 60 cap on 144 Hz lands at ~48); telemetry's `loopHz` reads the capped rate. Capture takes bypass the gate (`engineClock.fixed`).
...
- Each localStorage key is a separate string (not a single blob): `graphics-quality`, `render-scale`, `max-fps`, `music-volume`, `music-enabled`, `ui-visible`, `mouse-sensitivity`, `aim-sensitivity`, etc.
